...
def optimal_redispatch(load_per_node:pd.DataFrame, df_RD_orderbook:pd.DataFrame, Tee :int = 0) -> pyo.ConcreteModel:
    ...
    model.order_set = pyo.RangeSet(0,len(df_RD_orderbook)-1)
    ...
    # Define p
    def def_p(m, b, t):
        return m.p[b, t] == m.p_prognosis[b, t] + sum(m.dp[b, t, o] for o in m.order_set)
    ...
    def dp_balance(m, t):
        return sum(m.dp[b, t, o] for b in m.bus_set for o in m.order_set) == 0.0
...
def optimal_redispatch_congestion_cost(load_per_node:pd.DataFrame, df_RD_orderbook:pd.DataFrame, Tee :int = 0) -> pyo.ConcreteModel:
    ...
    model.total_congestion = pyo.Var(within=pyo.Reals) #sum of congestion
    model.total_costs = pyo.Var(within=pyo.NonNegativeReals)   #costs
...
#%% CBC function
def optimal_CBC(load_per_node:pd.DataFrame, df_CBC_orderbook:pd.DataFrame, congestion:float, chp_prog:pd.DataFrame, Tee:int = 0, ratio: int= ratio,) -> pyo.ConcreteModel:
    ...
    def def_p(m, b, t):
        chp_busses = np.unique(chp_prog.iloc[:, 0].values)  # Ensure it's a NumPy array
        if b in chp_busses:
