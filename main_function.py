# -*- coding: utf-8 -*-
"""
Created on Mon Jan 27 15:51:12 2025
...
def main_function(ratio_itterative: float, i, old_ratio:float, noise_mape:float = 0.0 ) -> float:  
    import pandas as pd
    import numpy as np
    import sys
    ...
    perform_deterministic_part = False
    if i == 0: #the first itteration always perfor deterministic part
        perform_deterministic_part = True
    if i != 0:
        if ratio_itterative != old_ratio: #in case it is not the first itration, only perform determnistic part whn the raito has changed
            perform_deterministic_part = True
